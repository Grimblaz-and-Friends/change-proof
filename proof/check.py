...
def run(
    ...
    *,
    ...
    try:
        ...
        base_tip_endpoint = f"repos/{repo}/commits/{quoted_base_ref}"
        base_tip_object = _object(github.get(base_tip_endpoint), base_tip_endpoint)
        base_tip = base_tip_object.get("sha")
        if not isinstance(base_tip, str) or not base_tip:
            raise ProofError("the base branch tip commit SHA is missing from the GET response")
